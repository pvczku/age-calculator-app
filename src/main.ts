...
const [dayInput, monthInput, yearInput, dayOutput, monthOutput, yearOutput, button] = [
  document.querySelector("#day") as HTMLInputElement,
  document.querySelector("#month") as HTMLInputElement,
  document.querySelector("#year") as HTMLInputElement,
  document.querySelector("#days") as HTMLSpanElement,
  document.querySelector("#months") as HTMLSpanElement,
  document.querySelector("#years") as HTMLSpanElement,
...
function checkInputs() {
  const [thisDay, thisMonth, thisYear]: number[] = [
    new Date().getDay(),
    new Date().getMonth() + 1,
  ...
  const correctNumbersInputs: boolean = correctNumberInputs(thisDay, thisMonth, thisYear);
...
function checkIfNumbers() {
  ...
  if (
    (yearInput.value !== "" && isNaN(Number(dayInput.value))) ||
...
function correctNumberInputs(thisDay: number, thisMonth: number, thisYear: number): boolean {
  let correctDay = false;
  let correctMonth = false;
  let correctYear = false;
  if (
    parseInt(dayInput.value) > monthDays[parseInt(monthInput.value)] ||
...
function calculateDifference(thisDay: number, thisMonth: number, thisYear: number): number[] {
  let days = 0;
  let months = 0;
  let years = 0;
